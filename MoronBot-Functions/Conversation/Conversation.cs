...
namespace Automatic
{
    public class Conversation : Function
    {
        ...
        public override List<IRCResponse> GetResponse(BotMessage message)
        {
            ...
            Match match = Regex.Match(message.MessageString, @"^('?sup|hi|hey|hello|greetings|bonjour|salut|howdy|'?yo|o?hai),?[ ]" + Regex.Escape(Settings.Instance.CurrentNick) + @"([^a-zA-Z0-9_\|`\[\]\^-]|$)", RegexOptions.IgnoreCase);
            if (match.Success)
            {
                return new List<IRCResponse>() { 
